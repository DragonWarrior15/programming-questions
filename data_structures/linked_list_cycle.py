...
class Solution:
    def hasCycle(self, head: Optional[ListNode]) -> bool:
        """When a node is visited, we modify its value to a constant, this
        acts as a marker. If we find a node with that value when traversing
        the list, there must be a cycle
        """
        const = 10**5 + 1
