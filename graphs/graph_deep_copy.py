...
class Solution:
    def cloneGraph(self, node: "Node") -> "Node":
        ...
        while node_stack:
            ...
            if node_list[curr_node.val - 1] != 0:
                continue
            # create the node
            new_node = Node(
                curr_node.val, neighbors=[x.val for x in curr_node.neighbors]
            )
            node_list[curr_node.val - 1] = new_node

            for x in curr_node.neighbors:
                node_stack.append(x)

        # now assign the neighbors as the nodes instead of values
        for i in range(len(node_list)):
            if node_list[i] != 0:
                node_list[i].neighbors = [
                    node_list[j - 1] for j in node_list[i].neighbors
                ]
