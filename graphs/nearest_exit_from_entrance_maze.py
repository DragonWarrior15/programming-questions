"""
https://leetcode.com/problems/nearest-exit-from-entrance-in-maze/

You are given an m x n matrix maze (0-indexed) with empty cells
(represented as '.') and walls (represented as '+'). You are also
given the entrance of the maze, where entrance = [entrancerow, entrancecol]
denotes the row and column of the cell you are initially standing at.

In one step, you can move one cell up, down, left, or right. You cannot
step into a cell with a wall, and you cannot step outside the maze. Your
goal is to find the nearest exit from the entrance. An exit is defined as
an empty cell that is at the border of the maze. The entrance does not count
as an exit.

Return the number of steps in the shortest path from the entrance to the
nearest exit, or -1 if no such path exists.

Example 1:
Input: maze = [["+","+",".","+"],[".",".",".","+"],["+","+","+","."]], 
entrance = [1,2]
Output: 1
Explanation: There are 3 exits in this maze at [1,0], [0,2], and [2,3].
Initially, you are at the entrance cell [1,2].
- You can reach [1,0] by moving 2 steps left.
- You can reach [0,2] by moving 1 step up.
It is impossible to reach [2,3] from the entrance.
Thus, the nearest exit is [0,2], which is 1 step away.

Example 2:
Input: maze = [["+","+","+"],[".",".","."],["+","+","+"]], entrance = [1,0]
Output: 2
Explanation: There is 1 exit in this maze at [1,2].
[1,0] does not count as an exit since it is the entrance cell.
Initially, you are at the entrance cell [1,0].
- You can reach [1,2] by moving 2 steps right.
Thus, the nearest exit is [1,2], which is 2 steps away.

Example 3:
Input: maze = [[".","+"]], entrance = [0,0]
Output: -1
Explanation: There are no exits in this maze.

Constraints:
    maze.length == m
    maze[i].length == n
    1 <= m, n <= 100
    maze[i][j] is either '.' or '+'.
    entrance.length == 2
    0 <= entrancerow < m
    0 <= entrancecol < n
    entrance will always be an empty cell.
"""


class Solution:
    def nearestExit(self, maze, entrance) -> int:
        m, n = len(maze), len(maze[0])

        def is_exit(row, col):
            # its an exit if we are at the boundary
            if row == 0 or row == m - 1 or col == 0 or col == n - 1:
                return True
            return False

        def is_entrance(row, col):
            if row == entrance[0] and col == entrance[1]:
                return True
            return False

        def is_inside_maze(row, col):
            if 0 <= row and row < m and 0 <= col and col < n:
                return True
            return False

        def is_empty(row, col):
            if maze[row][col] == ".":
                return True
            return False

        def is_wall(row, col):
            if maze[row][col] == "+":
                return True
            return False

        # do not visit already visited cells
        visited = {}

        # do a breadth first traversal until we reach the exits
        # we store the row, col, distance from entrance in the tuple
        queue = [(entrance[0], entrance[1], 0)]
        visited[(entrance[0], entrance[1])] = 1

        while queue:
            # print(queue)
            curr_row, curr_col, curr_dist = queue.pop(0)

            # mark as visited
            # visited[(curr_row, curr_col)] = 1

            # check if this is at the exit and is not the entrance
            if is_exit(curr_row, curr_col) and not is_entrance(curr_row, curr_col):
                return curr_dist

            # else try to add the neighbors of this cell
            for delta_row, delta_col in [[0, 1], [0, -1], [1, 0], [-1, 0]]:
                new_row, new_col = curr_row + delta_row, curr_col + delta_col
                if is_inside_maze(new_row, new_col):
                    if is_empty(new_row, new_col):
                        if (new_row, new_col) not in visited:
                            queue.append((new_row, new_col, curr_dist + 1))
                            # this is needed, else there are cycles
                            visited[(new_row, new_col)] = 1

        # if all paths are exhausted, no exit
        return -1
